...
## Events — ⚠️ Design complete (18 named), Implementation partial (5 generic, corrected July 25, 2026)

Categories: Gold events, HP-for-Gold trades, Curse card rewards, Risk events, Classic reworked.

**Correction:** this section previously read "✅ Complete (18 events) — all implemented in prototype." That was false — confirmed by direct code inspection: only **5 generic events** are actually implemented in `js/data.js` (Ancient Tome, The Offering, Hidden Cache, The Gambler's Curse, The Blood Pool) — none share a title or exact mechanic with GDD.md's 18 named events. The 18 remain design-only; none have been built, in either the split build or the legacy `castle-run.html` monolith. This was a design-only gap, not a migration loss.

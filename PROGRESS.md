...
| Events | Design: 18 named (GDD.md) | Built: 5 generic (not the GDD 18) |
...
## Events — Design complete (18 named), Implementation partial (5 generic)

GDD.md designs 18 named events. Only 5 generic events are actually implemented in `js/data.js` (Ancient Tome, The Offering, Hidden Cache, The Gambler's Curse, The Blood Pool) — none share a title or exact mechanic with GDD's 18. The 18 named events remain design-only; none have been built. This is not a migration gap — they were never implemented, in either the split build or the legacy `castle-run.html` monolith.
...
| July 23, 2026 (follow-up) | Fixed the `lastFightWasElite` stale-flag root cause: `startBossFight()` now sets `G.lastFightWasElite = false` (it was only ever set by `startCombat()`, so the flag stayed stale-true through a boss fight that followed an elite). This resolves three "after elite" effects misfiring on such a boss — `iron_ration` (heal 5), `grave_robber` (+8 Gold), and Void Compass (3-relic screen). No separate guards needed on iron_ration/grave_robber; the Void Compass `!G.inBoss` guard was kept as harmless defensive redundancy. Also corrected a stale sub-note in `DESIGN_DISCREPANCIES.md`'s Burn section (tooltip is already ×1, not ×2). Traced elite→boss (none of the three fire on the boss) and elite→regular (they still fire on the elite). |
